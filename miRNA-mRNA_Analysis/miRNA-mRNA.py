...
def getCircrPath():
	c_path = os.path.realpath(__file__).replace("Circr_2.py","")
	return c_path
...
def examine(fileIn, DB, acronym):
    ...
    os.system('echo {0}'.format('mir_TMP_'+fileIn))
    return DB
...
def hybrid(fileIn):
	...
	os.system('echo {0}'.format('hyb_TMP_'+fileIn))
...
def TS_parser(fileIn, DB, acronym):
	...
	os.system('echo {0}'.format('TS_TMP_'+fileIn))
...
def RH_parser(fileIn, DB):
	...
	os.system('echo {0}'.format('hyb_Parsed_'+fileIn))
	return(pd.concat([DB,df]).reset_index(drop=True))
...
def calculate_exons(bedfile, gtf, rrna):
	...
	exons = df_data.loc[df_data['feature'] == 'UTR']
...
def create_circular_DB(exons, bed_dataframe, rrna_file, genome):
	...
	coord_final.to_csv('coord_final.csv', sep='\t')
	...
	os.system('echo output_fasta.fa')
...
def starting_from_coord(bedfile, rrna, genome):
	...
	for circ_name in circulars:
		Data['>'+circ_name] = ''.join(fasted.loc[fasted['circ_name']==circ_name]['sequence'].to_list())
	# sequences = [SeqRecord(Seq(sequence), id=sequence_id, description="") for sequence_id, sequence in Data.items()]
	# output_file = "output_full_seq.fa"
...
def third_party_processes(data_dict, taxa_number, cores, Targetscan_miRNA , mature_file, acronym, miranda_sc, miranda_en, miranda_scale, miranda_go, miranda_ge, rnahybid_max):
	...
	processes = []
	out_list = []
	output = 'miRNA_analysed_output.txt'
	out_list.append(output)
	file_path = getCircrPath()
	UTR = getUTR(taxa_number)

	inp = 'miRNA_inputfile.fa'
	TS_inp = 'TS_'+inp
	with open(inp, "w") as mirna_file:
    		for item in data_dict.keys():
        			mirna_file.write(item + "\n" + data_dict[item] + "\n")
	with open(TS_inp, "w") as TS_mirna_file:
		for item in data_dict.keys():
			TS_mirna_file.write(item.lstrip('>')+'\t'+str(taxa_number)+'\t'+data_dict[item]+'\n')
	...
	processes.append('/scr/user/mk_98/circR_CIRIquant_Superdepth_NS/miRNA_mRNA/tools/targetscan_70.pl {2} {0} {1}'.format('TS_'+inp, 'TS_TMP_'+output, Targetscan_miRNA))#<-- check support file for targetscan
...
############## END CODE #################
def main(bedfile, gtf, rrna, AGO, TAXA, outfile, Genome, cores, valid_interactions, TS_miRNA, mirna_mature, mirna_acronym, coord, genome_version,
	...
	circular_database = third_party_processes(circ_dict, TAXA, cores, TS_miRNA, mirna_mature, mirna_acronym, miranda_sc, miranda_en, miranda_scale, miranda_go, miranda_ge, rnahybrid_max)
...
if __name__ == "__main__":
	...
	#parser.add_argument("--TS_library", type=str,help="Targetscan library for input.", default="none")
	parser.add_argument("--TS_miRNA_lib", type=str,help="Targetscan miRNA input.", default="none")
	#parser.add_argument("--miRanda_library", type=str,help="miRanda library input.", default="none")
	...
	os.system('chmod +x /scr/user/mk_98/circR_CIRIquant_Superdepth_NS/miRNA_mRNA/tools/targetscan_70.pl'.format(getCircrPath()))
